...
import { CheckCircle, XCircle, Eye, ExternalLink, Wallet, Clock, Filter } from 'lucide-react'
...
export default function AdminTransactions() {
  ...
  const load = async () => {
    ...
    // Also load confirmed and failed
    const { data: allData } = await paymentService.getByUserId ? { data: [] } : { data: [] }
  ...
  const handleVerify = async (action) => {
    ...
    const ok = await confirm({
      ...
      message: isConfirm
        ? `Pembayaran sebesar ${formatIDR(selected.amount)} dari ${selected.profiles?.full_name} akan dikonfirmasi. Cicilan akan ditandai lunas.`
        : `Pembayaran ini akan ditolak. User perlu mengunggah ulang bukti transfer yang valid.`,
    ...
    const status = isConfirm ? 'confirmed' : 'failed'
    const { error } = await paymentService.verifyPayment(selected.id, status, admin?.id, verifyNotes)
    ...
    <DashboardLayout role="admin">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Konfirmasi bukti transfer dari user</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="card-premium p-4">
            ...
            <p className="text-xl font-800 text-slate-900 mt-1">{formatIDR(stats.totalAmount)}</p>
          </div>
          <div className="card-premium p-4">
            ...
            <p className="text-sm font-700 text-emerald-700">Sistem Pembayaran Aktif</p>
          </div>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <EmptyRow colSpan={7} message={filter === 'verification' ? 'Tidak ada pembayaran menunggu verifikasi' : 'Tidak ada data'} />
              ) : items.map(item => (
                <Tr key={item.id}>
                  <Td><span className="font-600 text-xs font-mono">{item.loans?.ref_number || item.gadai_applications?.ref_number || '-'}</span></Td>
                  <Td>
                    <p className="font-600 text-sm text-slate-900">{item.profiles?.full_name || '-'}</p>
                    <p className="text-xs text-slate-400">{item.user_id?.slice(0,8)}</p>
                  </Td>
                  <Td className="font-700 text-emerald-700">{formatIDR(item.amount)}</Td>
                  <Td className="text-xs">{item.payment_method || 'Transfer'}</Td>
              ...
              <ModalBody>
                ...
                <div className="space-y-3 mb-5">
                  {[
                    ...
                    { label: 'Tanggal Upload', value: formatDateTime(selected.created_at) },
                    { label: 'Metode Pembayaran', value: selected.payment_method || 'Transfer Bank' },
                    { label: 'Nomor Referensi Transfer', value: selected.transfer_ref || '-' },
                  ].map(({ label, value }) => (
