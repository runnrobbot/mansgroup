...
import { loanService, gadaiService, profileService } from '../../services'
import { formatIDR, formatRelativeTime } from '../../lib/utils'
import { CheckCircle, Users, Ban, ArrowRight, Package, Clock, AlertTriangle, ArrowLeftRight } from 'lucide-react'

const stagger = { visible: { transition: { staggerChildren: 0.06 } } }
const fadeUp = { hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { duration: 0.3 } } }

export default function AdminDashboard() {
  const [stats, setStats] = useState({ approvals: 0, gadaiApprovals: 0, users: 0, overdue: 0 })
  const [pendingLoans, setPendingLoans] = useState([])
  const [pendingGadai, setPendingGadai] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    Promise.all([
      loanService.listAll({ status: 'review', limit: 5 }),
      gadaiService.listAll({ status: 'review', limit: 5 }),
      profileService.listAll({ limit: 1 }),
      loanService.listAll({ status: 'overdue', limit: 1 }),
    ]).then(([loans, gadai, users, overdue]) => {
      setStats({
      ...
      setPendingGadai((gadai.data || []).slice(0, 5))
      setLoading(false)
    })
  }, [])
  ...
  const quickLinks = [
    ...
    { label: 'Transaksi', desc: 'Riwayat pinjaman & gadai', href: '/admin/transactions', icon: ArrowLeftRight },
        ...
        <motion.div variants={fadeUp} className="grid lg:grid-cols-2 gap-5">
          ...
          <Card>
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-700 text-slate-900">Antrian Final Approval — Pinjaman</h3>
          ...
          <Card>
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-700 text-slate-900">Antrian Final Approval — Gadai</h3>
                <p className="text-xs text-slate-400 mt-0.5">Gadai siap disetujui</p>
              </div>
              <Link to="/admin/approvals" className="text-xs font-600 text-violet-600 hover:text-violet-700 flex items-center gap-1">
                Lihat semua <ArrowRight size={12} />
              </Link>
            ...
            <div className="divide-y divide-slate-50">
              {loading ? [1,2,3].map(i => (
                <div key={i} className="px-5 py-3"><div className="skeleton h-4 rounded w-3/4" /></div>
              )) : pendingGadai.length === 0 ? (
                <div className="px-5 py-8 text-center text-sm text-slate-400">Tidak ada antrian</div>
              ) : pendingGadai.map(g => (
                <div key={g.id} className="px-5 py-3.5 flex items-center justify-between hover:bg-slate-50/60">
                  <div>
                    <p className="text-sm font-600 text-slate-900">{g.profiles?.full_name || '-'}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-slate-400">{g.ref_number}</span>
                      <span className="text-slate-300">·</span>
                      <span className="text-xs font-600 text-violet-700">{formatIDR(g.loan_amount)}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <StatusBadge status={g.status} />
                    <Clock size={12} className="text-slate-300" />
                    <span className="text-xs text-slate-400">{formatRelativeTime(g.created_at)}</span>
                  </div>
                </div>
              ))}
            </div>
